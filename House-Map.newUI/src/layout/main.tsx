...
import { Radio, Button,Grid,Form, Input,Select,List,Menu,Dropdown,Modal,Carousel,Spin } from '@arco-design/web-react';
...
const Main: FC = () => {
    ...
    //展示选中的房源数据弹窗
    const showSelectedHouseDataModal = (data) => {
      if (mobile) {
        // 如果是移动端，跳转到详情页
    ...
    const loadDataFrom = async (key,value) => {
    ...
    const loadMore = () => {
      const nextPageData = totalData.slice(searchParams.page * searchParams.pageSize, (searchParams.page + 1) * searchParams.pageSize);
      setList(list.concat(nextPageData));
      // 如果没有更多的数据可以加载，那么停止加载
      if ((searchParams.page + 1) * searchParams.pageSize >= totalData.length) {
        setScrollLoading('没有更多数据了');
      }
    ...
    const onFinish = async (values:searchParamsProps) => {
     console.log("values",values);
      ...
      setSearchParams({
        ...
        source:values.source||searchParams.source||null,
        rentType:values.rentType|| searchParams.rentType||null,
        fromPrice:values.formPrice||searchParams.fromPrice||null,
        toPrice:values.toPrice||searchParams.toPrice||null,
        page: 1,
      });
    };
    useEffect(() => {
      ...
      const dropList = (
        <Menu>
          <Menu.Item key='1' onClick={() => window.open('https://wj.qq.com/s/2953926/aabe', '_blank')}>帮我们做的更好</Menu.Item>
    ...
    <div style={{ width: '50%', height: '100%' }}>
      <Carousel
      style={{height:'85%'}}>
        {selectedHouseData.pictures && selectedHouseData.pictures.map((src, index) => (
          <div key={index}>
            <img
              src={src}
    ...
    <h2>{selectedHouseData.price=='-1'?'暂无房价':selectedHouseData.price}</h2>
    <div style={{display:'flex',flexDirection:'column',alignItems:'center', justifyContent:'space-between',height:'40%'}}>
  ...
  <p><a href={selectedHouseData.onlineURL}>在线链接</a></p>
    </div>
    </div>
  </div>
      </Modal>
            <Row className={styles.header} >
                <Col span={mobile?18:10}>
                    <a className={styles['logo-content']} href="https://wj.qq.com/s/2953926/aabe" target='_blank'>地图搜租房</a>
                </Col>
                <Col className={styles['channel-btn']} span={6}>
                <Button type='text' onClick={() => window.open('https://wj.qq.com/s/2953926/aabe', '_blank')}>帮我们做的更好</Button>
  ...
  <FormItem label='类型' field='rentType'>
    <Select placeholder='请选择类型' options={houseTypeOptions} />
  </FormItem>
  <FormItem label='价格区间'>
    <Grid.Row gutter={8}>
      <Grid.Col span={12}>
  ...
  <Col style={{display:'flex',justifyContent:'center'}}>
    <Button size="large" className={styles['search-button']} type="primary" htmlType='submit'>搜索</Button>
  </Col>
</Form>
                    </Box>
                ...
                <Radio.Group defaultValue={'全部'} name='button-radio-group'
                  style={{ display: 'flex', overflowX: 'scroll', whiteSpace: 'nowrap' , scrollbarWidth: 'none',
          ...
          {checkList.map((item) => {
            return (
              <Radio key={item.value} value={item.value}>
                {({ checked }) => {
                  return (
                    <Button tabIndex={-1} key={item.label} shape='round' type={checked ? 'primary' : 'default'}  onClick={() => loadDataFrom('source',item.value)}>
                      {item.label}
                    </Button>
                  );
                }}
                ...
                <List className={styles['card-list']}
      ...
      bordered={false}
      render={(item, index) => (
        <List.Item key={index} onClick={()=>showSelectedHouseDataModal(item)}>
          <HouseCard className={styles.card} {...item} key={item.id}/>
        </List.Item>
      )}
    />      
            </Box>
