...
function GetDataByIndex(index, pagecount) {
    $.ajax({
        ...
        data: { cnName: cityNameCNPY, index: index },
        success:
            function (result) {
                if (result.IsSuccess) {
                    ...
                    rent_locations.forEach(function (element, index) {
                        addMarkerByAddressAndMarkBG(element.HouseLocation, element.Money, element.HouseURL,
...
function AutoGet58houseData(map) {
    //实例化城市查询类
    var citysearch = new AMap.CitySearch();
    //自动获取用户IP，返回当前城市
    citysearch.getLocalCity(function (status, result) {
...
function addMarkerByAddressAndMarkBG(address, memoy, href, markBG) {
    var geocoder = new AMap.Geocoder({
        city: cityName,
        radius: 1000
    });
