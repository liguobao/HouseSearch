...
namespace HouseMap.Crawler
{
    ...
    public class SpaciousCrawler : BaseCrawler
    {
        ...
        public override List<BaseHouseInfo> ParseHouses(JToken config, string data)
        {
            ...
            {
                string houseLocation = GetLocation(element);
                string houseTitle, disPlayPrice;
                decimal housePrice;
                FillTitlePrice(element, houseLocation, out houseTitle, out housePrice, out disPlayPrice);
                DateTime pubTime = GetPubtime(element);

                string onlineUrl = GetOnlineURL(element);
        ...
        private static string GetLocation(IElement element)
        {
        ...
        private static string GetOnlineURL(IElement element)
        {
            return "https://www.spacious.hk" + element.QuerySelectorAll("a")
            .Where(item => item.ClassList.Contains("GTM-tracking-full-listing"))
            .FirstOrDefault()?.GetAttribute("href");
        }

        private static DateTime GetPubtime(IElement element)
        {
        ...
        private static void FillTitlePrice(IElement element, string houseLocation,
         out string houseTitle, out decimal housePrice, out string disPlayPrice)
        {
            houseTitle = "";
            housePrice = 0;
            disPlayPrice = "";
            var titleItem = element.QuerySelectorAll("div").Where(div => div.ClassList.Contains("cell-listing-carousel--infobox-text"))
            .FirstOrDefault();
            if (titleItem == null)
            {
                return;
