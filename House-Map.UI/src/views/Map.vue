  ...
  <div class="map" :class="{ 'showInfo': isMobile }">
    <header v-if="!isMobile">
      ...
      <div class="search-wrap">
        <div class="key-word" v-if="!isMyCollections">
          <div class="keyword-tag">
            <el-tag class="tag" :key="tag" v-for="tag in keywordArr" closable :disable-transitions="false"
              @close="removeKeyword(tag)">{{ tag }}</el-tag>
          </div>
          <el-input class="keyword-tag-input" v-model="keywordTag" ref="saveTagInput" size="small" v-if="inputVisible"
            @keyup.enter.native="keywordConfirm" @blur="keywordConfirm" placeholder="搜索关键字" :maxlength="50"></el-input>
          <el-button v-if="!inputVisible && keywordArr.length <= 6" class="button-new-tag" size="small"
            @click="showInput">+ 关键词</el-button>
        </div>
        <el-button v-if="!isMyCollections" type="primary" class="search-btn" size="small" @click="refresh"
          :loading="loading">搜索</el-button>
        <el-checkbox v-model="isMyCollections" v-if="user" :disabled="loading" style="margin-left: 10px;"
          @change="updateMyCollectionState">我的收藏</el-checkbox>
      </div>
    </header>
    <div class="filter-wrap" v-if="!isMobile && !isMyCollections">
      <ul>
        <li>
          <span>房源类型：</span>
          <div>
            <el-select v-model="form.rentType" @change="refresh" placeholder="请选择房源类型" size="mini" style="width: 100%">
              <el-option :key="item.value" v-for="item in rentTypeArr" :label="item.label"
          ...
          <div class="displaySource">
            <span :class="{ active: !$route.query.source }" @click="selectSource('')">全部</span>
            <span :class="{ active: item.source === $route.query.source }" v-for="item in source " :key="item.source"
              @click="selectSource(item.source)">{{ item.displaySource }}
            </span>
    ...
    <div class="map-content" v-loading="loading">
      <div v-if="!isMobile" class="map-house-list">
        ...
        <el-collapse-transition>
          <house-list :loading="loading" height="100%" v-show="toggleHouseListUp" :house-list="mapHouseList"
            @click="houseListClick"></house-list>
        </el-collapse-transition>
      </div>
      <div class="container-wrap">
        ...
        <template v-if="!isMobile && !isMyCollections">
          ...
          <el-collapse class="collapseCard">
            <!-- <h4>出行到达圈查询</h4> -->
            <el-collapse-item title="出行导航(点击展开)">
              <div class="card">
                <div class="card-item">
                  <span class="card-name">上班地点</span>
                  <el-input id="keyword" class="card-value" size="mini" type="text" placeholder="请输入内容"
                ...
                <div class="card-item">
                  ...
                  <el-select v-model="waysMethod" placeholder="请选择出行方式" class="card-value" size="mini">
                    <el-option label="地铁+公交" value=""></el-option>
          ...
          <span class="highlight-text">
            特此声明:房源信息来自网络，请自行鉴别。合租需谨慎，群租有风险，长租公寓多当心。地图首次载入较慢。
        ...
        <template v-if="!inMap">
          <div class="filter">
            <div class="filter-item">
              <span>上班地点:</span>
              <el-input id="keyword" class="card-value" size="mini" placeholder="请输入内容" v-model="keyword"
                clearable></el-input>
            </div>
            <div class="filter-item">
          ...
          <div class="mobile-bg" v-if="makerInfo" @click="handleMobileBg">
            <div class="content">
              <section>
                ...
                <a :href="`/#/detail/${makerInfo.id}`" target="_blank" class="content-value">{{ makerInfo.title }}</a>
              </section>
              <section v-if="makerInfo.displayMoney">
                <span class="content-value">{{ makerInfo.displayMoney }}</span>
              </section>
              <section>
                <el-tag size="mini" :type="tagType(makerInfo)">{{ tagText(makerInfo) }}</el-tag>
              </section>
              <section>
                <span class="content-value">{{ makerInfo.sourceContent }}</span>
              </section>
              <section v-if="makerInfo.pictures && makerInfo.pictures.length">
                <span class="content-name btn" @click="preview(makerInfo)">
                  <i class="el-icon-picture"></i>查看图片
                </span>
              </section>
              <section>
                <span class="content-name btn" @click="collect(makerInfo)">
                  <i class="el-icon-star-on"></i>收藏
                </span>
              </section>
              <section>
                <span class="content-name btn" @click="navTo(makerInfo)">
                  <i class="el-icon-location"></i>开始导航
                </span>
              </section>
            </div>
          </div>
...
export default {
  ...
  watch: {
    "$route.query": function (params) {
      if (
        (!params.mobileType || params.mobileType !== "list") &&
        this.isMobile
      ) {
        this.loading = true;
        this.initMark();
  ...
  methods: {
    ...
    async updateLatLng(list, params) {
      if (!list || !list.length) {
        return;
      }
      const data = await this.$ajax.put(`v2/houses-lat-lng`, list);
      setTimeout(() => {
        this.housesRefresh(params);
      }, 2000);
    },
    selectSource(source) {
      if (this.loading) {
    ...
    async cityChange(cityName) {
      const data = await this.$ajax.get(`v2/cities/${cityName}`);
    ...
    refresh(q) {
      ...
      let query = this.$route.query;
      let params = Object.assign({}, query, this.transformParams(), q2);
    ...
    cityLocation(item) {
      ...
      this.activeCityName = this.cityName = item;
      this.geocoder.setCity(item);
      this.initCenter(this.posPicker, this.geocoder, this);
      this.initMap();
      this.refresh();
    },
    keywordConfirm() {
      ...
      self.arrivalRange.search(
        ...
        function (status, result) {
          ...
          if (result.bounds) {
            ...
            self.map.setFitView();
            self.map.setZoomAndCenter(self.isMobile ? 15 : 13, self.lnglat);
    ...
    houseListClick(item) {
      if (item.geocodes && item.geocodes.location && item.geocodes.location.lng) {
    ...
    async getList(params) {
      let data = await this.$ajax.post("v2/houses", params);
    ...
    getLocation(code, item) {
      return new Promise((resolve, reject) => {
        let location = this.stripscript(item.location);
        code.getLocation(location, (status, result) => {
          resolve({
            status,
            result,
          });
    ...
    addMaker(map, data, code, self, massMarkStyle) {
            ...
            return new Promise(async (resolve, reject) => {
              ...
              try {
                ...
                if (
                  status === "complete" &&
                  result.info &&
                  result.info.toLocaleLowerCase() === "ok"
                ) {
                  let style = 1;
                  if (item.icon) {
                    style = massMarkStyle.findIndex((s) => {
                      return item.icon.indexOf(s.type) !== -1;
                    });
                  }
                  ...
                  let displayMoney =
                    item.price > 0 ? "租金：" + item.price : "";

                  let sourceContent = item.source
                    ? " 来源：" + item.source
                    : "";
    ...
    keywordSelect(map, positionPicker) {
      ...
      let self = this;
      autocomplete.on('select', selectChange);
      // 废弃方法 AMap.event.addListener(autocomplete, "select", select); //注册监听，当选中某条记录时会触发
    ...
    getCityCenter(positionPicker, code, self, name, cb) {
      ...
      return new Promise((resolve, reject) => {
        code.getLocation(name, (status, result) => {
          // 城市中心点
          if (status === "complete" && result.info === "OK") {
            location = result.geocodes[0].location;
            positionPicker.start(location);
            self.lnglat = AMap.LngLat(location.lng, location.lat);
            ...
            if (cb) {
              ...
              self.lnglat = new AMap.LngLat(location.lng, location.lat);
    ...
    async init(location) {
      ...
      try {
        ...
        this.posPicker = await this.installAmapUI(map, self);
        let positionPicker = this.posPicker;
        ...
        let code = this.geocoder;

        this.initCenter(positionPicker, code, self);

        this.keywordSelect(map, positionPicker);

        this.massMarkStyle = [
          {
            url: require("./../images/Black.png"),
          ...
          {
            url: require("./../images/LightGreen.png"),
            anchor: new AMap.Pixel(3, 3),
            size: new AMap.Size(19, 30),
            type: "LightGreen",
          },
          {
            url: require("./../images/OrangeYellow.png"),
            anchor: new AMap.Pixel(3, 3),
            size: new AMap.Size(19, 30),
            type: "OrangeYellow",
          },
          {
            url: require("./../images/PaleGreen.png"),
            anchor: new AMap.Pixel(3, 3),
            size: new AMap.Size(19, 30),
            type: "PaleGreen",
          },
          {
            url: require("./../images/PaleRed.png"),
            anchor: new AMap.Pixel(3, 3),
            size: new AMap.Size(19, 30),
            type: "PaleRed",
          },
          {
            url: require("./../images/PaleYellow.png"),
            anchor: new AMap.Pixel(3, 3),
            size: new AMap.Size(19, 30),
            type: "PaleYellow",
          },
          {
            url: require("./../images/Pink.png"),
            anchor: new AMap.Pixel(3, 3),
            size: new AMap.Size(19, 30),
            type: "Pink",
          },
    ...
    async initCenter(positionPicker, code, self) {
      if (
        this.$store.state.userInfo &&
        this.$store.state.userInfo.workAddress
      ) {
        await this.getCityCenter(
          positionPicker,
          code,
          self,
          this.$store.state.userInfo.workAddress,
    ...
    async initMark(arrayData = []) {
      return new Promise(async (resolve, reject) => {
        ...
        let params = {
          ...this.$route.query,
          city: this.cityName.replace(/市/gi, ""),
          houseCount,
        };
        delete params.cityname;
        let info = await this.getList(params);
        ...
        let massMarkStyle = this.massMarkStyle;
        let newData = await this.addMaker(map, data, code, self, massMarkStyle);

        let dataArr = newData.filter((item) => {
          if (item.lnglat) {
            return item;
          }
        });
        ...
        //   }))
        // });
        let massMarks = undefined;

        if (!this.massMarks) {
          massMarks = new AMap.MassMarks([], {
            zIndex: 120, // 海量点图层叠加的顺序
            zooms: [3, 19], // 在指定地图缩放级别范围内展示海量点图层
            style: massMarkStyle, //多种样式对象的数组
          });
        ...
        massMarks.on("click", function (e) {
          let eData = e.data;
          let item = eData.item;
          let displayMoney = eData.displayMoney;

          let sourceContent = eData.sourceContent;
          let title = eData.title;
          let publishDate = eData.publishDate;

          if (self.isMobile) {
            self.makerInfo = eData.params;
          } else {
                ...
                return h(
                  ...
                  [
                    h("a", {
                      attrs: {
                        target: "_blank",
                        href: `${self.origin}#/detail/${item.id}?onlineURL=${item.onlineURL
                          }`,
                      },
                      class: ["marker-link"],
                      domProps: {
                    ...
                    h("a", {
                      ...
                      class: ["marker-link"],
                      domProps: {
                        innerHTML: `发布时间: ${item.publishDate}`,
                      },
                    }),
                    h("a", {
                      attrs: {
                    ...
                    h("a", {
                      attrs: {
                        target: "_blank",
                        href: `${self.origin}#/Map?city=${item.city}&source=${item.source
                          }&rentType=${item.rentType}`,
                      },
                      class: ["marker-link"],
                      domProps: {
                        innerHTML: `类型: ${rentType}`,
                      },
                    }),
                    h("a", {
                      attrs: {
                    ...
                    h(
                      "span",
                      {
                        ...
                        on: {
                          click: function (e) {
                            self.transfer(eData.lnglat, map, self);
                            infoWindow.close();
                          },
                        },
                      },
            ...
            infoWindow.open(map, eData.lnglat);
          }
        });

        massMarks.setMap(map);
        massMarks.setData(dataArr);

        // new AMap.MarkerClusterer(map, markers, {gridSize: 100});

        dataArr = dataArr.filter((item) => {
          return item.update;
        });
        let updateList = dataArr.map((item) => {
          return {
            id: item.item.id,
            longitude: item.lnglat.lng,
            latitude: item.lnglat.lat,
            source: item.item.source,
            onlineURL: item.item.onlineURL,
            city: item.item.city,
          };
        });
        this.updateLatLng(updateList, params);
        setTimeout(() => {
          this.search("auto");
        });
        this.loading = false;
    ...
    async housesRefresh(params) {
      const data = await this.$ajax.post(`v2/houses-refresh`, params);
    },
    async getMyCollections() {
      const userId = this.$store.state.userInfo.id;
      const data = await this.$ajax.get(`v2/users/${userId}/collections`);
      if (Array.isArray(data.data) && data.data.length) {
        this.initMark(data.data);
      }
    },
  },
  async created() {
  ...
  async mounted() {
    ...
    if (cityName) {
      ...
      this.$router.replace({ query: { city: "上海" } });
      window.location.reload();
      return;

    }

    let key = `8b942f01c2c6b908b8594f86c07c1725`;
    let plugin = `AMap.ArrivalRange,AMap.Scale,AMap.Geocoder,AMap.Transfer,AMap.AutoComplete,AMap.CitySearch,AMap.Walking,AMap.MarkerClusterer`.split();
    plugin.push(`AMap.ToolBar`);
    let url = `https://webapi.amap.com/maps?v=2.0&key=${key}&plugin=${plugin.join()}`;

    userInfo(this);
    await this.appendScript(url);
    await this.appendScript(`//webapi.amap.com/ui/1.1/main.js?v=1.1.1`);
    //await this.appendScript(`//cache.amap.com/lbs/static/addToolbar.js`);
