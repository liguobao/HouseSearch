<template>
  <div class="dashboards" :class="{'is-mobile':isMobile}">
    <template v-if="!loading">
      <div class="search" v-if="type === 'all'">
        <el-input placeholder="城市名" prefix-icon="el-icon-search" @input="search" v-model="key"></el-input>
      </div>
  ...
  methods: {
    async getData() {
      ...
      if (this.dataType === "all") {
        const res = await this.$ajax.get("v2/cities");
        let data = res.data;
        this.cities = data;
      } else {
        const userId = this.$store.state.userInfo.id;
        const res = await this.$ajax.get(
          `v2/users/${userId}/collections/city-source`
        );
        const data = res.data;
        this.cities = data;
      }
