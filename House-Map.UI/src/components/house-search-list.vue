<template>
  <div>
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column type="expand" width="80" label="图片">
        <template slot-scope="props">
          <div class="carousel">
            <el-carousel
              ...
              height="150px"
              :autoplay="false"
              v-if="props.row.pictures && props.row.pictures.length"
              arrow="always"
            >
                ...
                <div class="carousel-image">
                  ...
                  <transition name="el-fade-in">
                    <a
                      ...
                      v-show="imagesLoadingMap[item]"
                      :href="item"
                      title="查看原图"
                    >
                      <img :src="item" @load="imageLoading(item,imagesLoadingMap)">
                    </a>
                  </transition>
                </div>
              </el-carousel-item>
      ...
      <el-table-column align="center" prop="city" label="城市" width="80"></el-table-column>
      <el-table-column align="center" prop="title" label="标题" width="300">
        <template slot-scope="scope">
          <!--<a slot="reference" class="ellipsis text-left link" :href="scope.row.onlineURL" target="_blank">{{-->
          <!--scope.row.title }}</a>-->
          <router-link
            slot="reference"
            class="ellipsis text-left link"
      ...
      <el-table-column align="center" prop="location" width="150" label="坐标">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" width="150" sortable prop="price" label="价格(元)">
        <template slot-scope="scope">
          <span class="" v-if="scope.row.price >= 0">{{ scope.row.price }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" width="180" prop="labels" label="标签">
        <template slot-scope="scope">
      ...
      >
        <template slot-scope="scope">
          ...
          <span class="pub-time">{{ $transformData(scope.row.pubTime,'yyyy-MM-dd hh:mm:ss') }}</span>
        </template>
      </el-table-column>
      <!--<el-table-column-->
      <!--align="center"-->
      ...
      <!--label="来源">-->
      <!--</el-table-column>-->
      <el-table-column v-if="type === 'user'" width="120" align="center" label="操作">
        <template slot-scope="scope">
          <el-button
            type="danger"
            size="small"
      ...
      <el-table-column v-else width="120" align="center" label="操作">
        <template slot-scope="scope">
          <el-button
            type="info"
            size="small"
  ...
  methods: {
    ...
    async del(row, index) {
      ...
      const data = await this.$ajax.delete(
        `v2/users/${userId}/collections/${row.id}`
      );
