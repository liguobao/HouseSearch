    ...
    >
      <el-form-item :label="isMobile ? '' : '地区'" prop="city">
        ...
        >
          ...
          <el-option v-for="item in cities" :key="item.id" :label="item.city" :value="item.city"></el-option>
        </el-select>
      </el-form-item>
      ...
      rules: (() => {
        const isNum = (rule, value, callback) => {
          if (this.form.fromPrice || this.form.toPrice) {
            const re = /^[0-9]+.?[0-9]*$/;
            if (
              (this.form.fromPrice && !re.test(this.form.fromPrice)) ||
              (this.form.toPrice && !re.test(this.form.toPrice))
            ) {
              callback(new Error());
            } else {
              callback();
  ...
  methods: {
    ...
    async getHousesList(options, type) {
      ...
      const data = await this.$ajax.get(
        `v2/houses?${this.$qs.stringify(params)}`
      );
    ...
    async getCities() {
      const data = await this.$ajax.get("v2/cities?fields=id,city");
