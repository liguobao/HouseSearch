        ...
        <ul>
          ...
          <li>
            <el-dropdown class="dropdown">
              <span class="el-dropdown-link nav-item">
                <template v-if="user">{{$store.state.userInfo.userName}}</template>
              ...
              <el-dropdown-menu slot="dropdown">
                <template v-if="user">
                  <el-dropdown-item>
                    <a href="javascript:" class="link-to" @click="showDashboards('user')">房源看板</a>
                  </el-dropdown-item>
                  <!--<el-dropdown-item><a href="javascript:" class="link-to" @click="getUserHouseList">收藏列表</a>-->
                  <!--</el-dropdown-item>-->
                  <el-dropdown-item disabled>
                    <a href="javascript:" class="link-to">绑定QQ</a>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <a href="javascript:" class="link-to" @click="setAddress">设置地址</a>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <a href="javascript:" class="link-to" @click="setEmail">设置邮箱</a>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <a href="javascript:" class="link-to" @click="logOut">退出登录</a>
                  </el-dropdown-item>
                </template>
                <template v-else>
                  <el-dropdown-item>
                    <a :href="oauthUrl ? oauthUrl : 'javascript:'" class="link-to">QQ登录</a>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <a href="javascript:" class="link-to" @click="login('login')">邮箱登录</a>
                  </el-dropdown-item>
                </template>
              </el-dropdown-menu>
            </el-dropdown>
          ...
          <li>
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item>
                  <a href="javascript:" class="link-to" @click="getHistoryNotices">历史公告</a>
                </el-dropdown-item>
                <el-dropdown-item>
                  <a href="javascript:" class="link-to" @click="scrollTo('contact')">联系我？</a>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </li>
          <li>
            <a
              href="https://mp.weixin.qq.com/s/Vzu4fsFspSvNqvOJ0kWfyg"
              target="_blank"
              class="nav-item"
            >关于我们</a>
          </li>
    ...
    >
      <div class="history-notices">
        <el-collapse accordion>
          <el-collapse-item :name="item.id" v-for="item in historyNotices" :key="item.id">
            <template slot="title">
...
<script>
const asyncComponent = require("./async-component.tsx").default;
...
export default {
  ...
  methods: {
    async setEmail() {
      let com = require("./set-email").default;
      try {
    ...
    navTo(item) {
      ...
      let { href } = this.$router.resolve({
        path: `/Map?${this.$qs.stringify(params)}`
      });
      window.open(href, "_blank");
    },
    historyTitle(item) {
      const name = item.content.slice(0, 80);
      const time = this.$transformData(
        item.dataCreateTime,
        "yyyy-MM-dd hh:mm:ss"
      );
      return `${name}... (${time})`;
    ...
    async setAddress() {
      ...
      this.$prompt(text, "设置上班地址", {
        ...
        .then(async ({ value }) => {
          ...
          await this.$ajax.post(`v1/users/${userId}/address`, {
            address: value
          });
    ...
    async getHistoryNotices() {
      const data = await this.$ajax.get("v1/notices");
  ...
  async created() {
    const data = await this.$ajax.get("/v1/account/oauth-url");
