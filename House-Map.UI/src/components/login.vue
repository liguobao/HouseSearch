<template>
  <div class="login">
    <div class="words text-center">
      <template v-if="type === 'login'">登录后查看个人收藏房源</template>
      <template v-else-if="type === 'register'">注册后可以收藏房源信息</template>
      <br>房子是租来的,而生活不是。
    </div>
    <el-form ref="form" :model="form" label-width="100px" :rules="rules">
      ...
      <el-collapse-transition>
        <div v-if="type === 'find-password'">
          <el-form-item label="邮箱地址" prop="email">
            <el-input v-model.trim="form.email" placeholder="请输入登录邮件接收重置密码邮件。" :maxlength="100"></el-input>
          </el-form-item>
        </div>
      </el-collapse-transition>
      <el-form-item label="" label-width="0px">
        <div class="btn-wrap text-center">
          <el-button
            ...
            :loading="loading"
            class="submit"
          >{{type === 'register' ?'注册' :type === 'find-password' ? '开始重置':'登录'}}</el-button>
        </div>
      </el-form-item>
    </el-form>
    <div class="text-center">
      <template v-if="type === 'register'">已有帐号？
        <el-button type="text" @click="type = 'login'">登录</el-button>
      </template>
      <template v-else-if="type === 'login'">没有帐号？
        <el-button type="text" @click="type = 'register'">注册</el-button>
        <br>忘记密码？
        <el-button type="text" @click="type = 'find-password'">重置密码</el-button>
      </template>
    </div>
    <div class="line"></div>
    <div class="third-party">
      <a :href="oauthUrl ? oauthUrl : 'javascript:'">
          ...
          userName: [
            {
              required: true,
              message:
                this.type === "login" ? "请输入账号" : "请输入邮箱或者昵称",
              trigger: "change",
            },
  ...
  methods: {
    ...
    async login() {
      try {
        ...
        const data = await this.$ajax.post("/v1/account", params);
    ...
    async register() {
      try {
        ...
        const params = Object.assign({}, this.form);
        delete params.checked;
        const data = await this.$ajax.post("/v1/account/register", params);
        this.$store.dispatch("UserLogin", data);
    ...
    async findPassword() {
      try {
        ...
        const params = Object.assign({}, this.form);
        delete params.checked;
        const data = await this.$ajax.post("/v1/account/find-password", params);
        this.$store.dispatch("UserLogin", data);
        this.$message.success(
          data.message ? data.message : "发送【重置密码】邮件成功。"
        );
        this.loading = false;
        this.done(data);
        this.$emit("close", "loginVisible", false);
